...
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
    ...
    // Apply trail color only if progress is less than 1.0 (i.e., within DURATION)
    if (progress < 1.0) {
        // Get the trail's primary axis (vector from left to right)
        vec2 trailAxis = v3 - v0;
        float trailLength = length(trailAxis);
        
        // Project fragment position onto the trail's axis
        vec2 toFragment = vu.xy - v0;
        float t = dot(toFragment, trailAxis) / (trailLength * trailLength);
        t = clamp(t, 0.0, 1.0); // Ensure t is between 0.0 (left) and 1.0 (right)
        
        // Gradient: 1.0 at left, 0.0 at right
        float gradient = 1.0 - t;
        gradient = smoothstep(0.0, 1.0, gradient); // Smooth the transition
        
        // Apply gradient to trail transparency
